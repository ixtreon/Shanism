    ...
    class AnimationTree : TreeView
    {
        ...
        void onDragDrop(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(typeof(TreeNode)))
            {
                ...
                if (AnimPath.IsSubFolderOf(destFolder, srcName))
                {
                ...
                {
                    var relName = AnimPath.GetRelativePath(an.Name, srcFolder);
                    var newName = AnimPath.Combine(destPath, relName);
        ...
        void selectHoverNode() =>
            SelectedNode = GetNodeAt(PointToClient(Cursor.Position)) ?? SelectedNode;
        ...
        void pasteNode()
        {
            ...
            var destName = AnimPath.Combine(destNode.Name, clipboardedNode.Text);
            var i = 1;
        ...
        {
            var segments = AnimPath.SplitPath(anim.Name);
            ...
            for (var i = 0; i < segments.Length; i++)
            {
                ...
                var curPath = AnimPath.Combine(segments.Take(i + 1));
        ...
        bool finishRename(TreeNode renamedNode, string newLastName)
        {
            ...
            {
                //an animation
                var fullName = AnimPath.Combine(renamedNode.Parent.Name, newLastName);
            ...
            else
            {
                ...
                var newPath = AnimPath.Combine(renamedNode.Parent.Name, newLastName);

                foreach (var n in renamedNodes)
                {
                    var nAnim = (AnimationViewModel)n.Tag;
                    var relName = AnimPath.GetRelativePath(nAnim.Name, renamedNode.Name);
                    var fullName = AnimPath.Combine(newPath, relName);
