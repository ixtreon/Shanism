...
namespace MGWinForms
{
    public abstract class GraphicsDeviceControl : GLControl
    {
        ...
        private string BeginDraw()
        {
            ...
            var control = Control.FromHandle(GraphicsDevice.PresentationParameters.DeviceWindowHandle) as GLControl;
            if (control != null)
            {
            ...
            var viewport = new Viewport(0, 0, ClientSize.Width, ClientSize.Height, 0, 1);
        ...
        private string HandleDeviceReset()
        {
            ...
            if (needsReset)
            {
                try
                {
                    _deviceService.ResetDevice(Handle, ClientSize.Width, ClientSize.Height);
                }
                catch (Exception e)
                {
