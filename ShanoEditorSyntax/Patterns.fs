...
//    let (|SyntaxToken|_|) (n: SyntaxToken) = 
//        match n with
//        |
...
//        match n with
//        | :? SwitchLabelSyntax as t -> Some(t.Kind(), t.Keyword, t.Value, t.ColonToken)
//        | _ -> None
// 
...
//        | _ -> None
    ...
    let (|SyntaxListTy|_|) (ty:System.Type) = 
        if ty.IsGenericType && ty.GetGenericTypeDefinition() = typedefof<SeparatedSyntaxList<_>>
        then Some (ty.GetGenericArguments().[0])
        else None

    let(|SyntaxList|_|) (n: CSharpSyntaxNode) = 
        match n.GetType() with
        | SyntaxListTy(t) when t = typeof<Syntax.ArgumentSyntax> ->
            Some()
        | _ -> 
            None

//        match n with
//        | :? SeparatedSyntaxList<T> as t when 
//
//         typedefof<Syntax.ArgumentSyntax>.IsAssignableFrom(n.GetType().GetGenericArguments().[0]) -> Some(1)
//        | _ -> None