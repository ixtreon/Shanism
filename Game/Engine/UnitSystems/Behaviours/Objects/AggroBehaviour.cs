...
namespace Shanism.Engine.Systems.Behaviours
{
    ...
    class AggroBehaviour : BehaviourList
    {
        ...
        Unit pickTarget()
        {
            ...
            // remove dead targets. 
            var toRemove = aggroTable.Where(kvp => kvp.Key.IsDead).ToList();
            double outVal;
            ...
            double maxAggro;
            var candidates = getMaxAggroGuys(aggroTable, out maxAggro);

            var newTarget = getClosestGuy(Owner.Position, candidates);

            return newTarget;
        }

        static List<Unit> getMaxAggroGuys(IEnumerable<KeyValuePair<Unit, double>> vals, out double maxAggro)
        {
            var cList = new List<Unit>();
            maxAggro = 0;

            foreach (var kvp in vals)
            {
                if (Math.Abs(kvp.Value - maxAggro) < 1e-5)
                {
                    cList.Add(kvp.Key);
                }
                else if (kvp.Value > maxAggro)
                {
                    cList.Clear();
                    cList.Add(kvp.Key);
                    maxAggro = kvp.Value;
        ...
        static Unit getClosestGuy(Vector loc, IEnumerable<Unit> units)
        {
            var minDist = double.MaxValue;
            Unit minGuy = null;
            foreach (var u in units)
            {
                var dist = loc.DistanceToSquared(u.Position);
                if (dist < minDist)
                {
                    minDist = dist;
                    minGuy = u;
